...
async def link_generator(client: Client, message: Message):
    while True:
        try:
            channel_message = await client.ask(text = "Fᴏʀᴡᴀʀᴅ Mᴇssᴀɢᴇ ғʀᴏᴍ ᴛʜᴇ DB Cʜᴀɴɴᴇʟ (ᴡɪᴛʜ Qᴜᴏᴛᴇs)..\nᴏʀ Sᴇɴᴅ ᴛʜᴇ DB Cʜᴀɴɴᴇʟ Pᴏsᴛ ʟɪɴᴋ", chat_id = message.from_user.id, filters=(filters.forwarded | (filters.text & ~filters.forwarded)), timeout=60)
        except:
            return
        ...
        if msg_id:
            ...
            await channel_message.reply("❌ Eʀʀᴏʀ\n\nᴛʜɪs Fᴏʀᴡᴀʀᴅᴇᴅ Pᴏsᴛ ɪs ɴᴏᴛ ғʀᴏᴍ ᴍʏ DB Cʜᴀɴɴᴇʟ ᴏʀ ᴛʜɪs Lɪɴᴋ ɪs ɴᴏᴛ ᴛᴀᴋᴇɴ ғʀᴏᴍ DB Cʜᴀɴɴᴇʟ", quote = True)
            continue
